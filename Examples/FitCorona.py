...
if ReduceDistricts:
    ...
    CumulCases = RawCumulCases[:, SelectedIDs, SelectedAges, SelectedGender]
    CumulDead = RawCumulDead[:, SelectedIDs, SelectedAges, SelectedGender]
    Cases = RawCases[:, SelectedIDs, SelectedAges, SelectedGender]
    Dead = RawDead[:, SelectedIDs, SelectedAges, SelectedGender]
    Cured = RawCured[:, SelectedIDs, SelectedAges, SelectedGender]
...
I0 = M.newVariables({'I0': 3.0e-7}, forcePos=False)  # a global variable of initial infection probability
...
r0 = M.newVariables({'r0': 2.0 * np.ones(M.Axes['Age'].shape, stm.CalcFloatStr)}, forcePos=True)
...
measured = CumulCases[:, :, :, 0:2] / PopSum
measuredDead = CumulDead[:, :, :, 0:2] / PopSum
NIter = 400
...
M.showStates(MinusOne=('S'), dims2d=None)
